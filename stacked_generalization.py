...
#    X_test = X[dev_cutoff:]
#    Y_test = Y[dev_cutoff:]
    X_dev=X_train.as_matrix()
    X_test= X_test.as_matrix()
    Y_dev= y_train.as_matrix()
    ...
    for j, clf in enumerate(clfs):
        ...
        for i, (train_index, cv_index) in enumerate(skf):
            ...
            X_train,Y_train=u.test_rest(X_train,Y_train,ratio=2,c=0)
            X_train,Y_train=u.test_smote(X_train,Y_train,c=0)
            X_cv = X_dev[cv_index]
            Y_cv = Y_dev[cv_index]
    ...
    score = u.roc_auc_score(Y_test, Y_test_predict)
...
#    data = data[1:] # remove header
    ...
    for i in xrange(5):
        print 'Iteration [%s]' % (i)
#        random.shuffle(data)
        score = run(X_train, X_test, y_train, y_test)
